...
def values_extraction_vina():
    cnt=0
    cnt_no=0
    fil_r_txt=open(sys.argv[1] + "vina_log/" + sys.argv[2] + ".pdbqt", "r")
    ...
    Kimolar = Ki*(0.000001)
    fil_r_pdbqt=open(sys.argv[1] + "vina_pdbqt/" + sys.argv[2] + ".pdbqt", "r")
    ...
    fil_w_txt=open(sys.argv[1] + "n_output.tsv", "w")
    for line in fil_r_txt:
    ...
    os.rename(sys.argv[1] + "n_output.tsv", sys.argv[1] + "output.tsv")
