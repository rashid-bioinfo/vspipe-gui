...
class VSPipeInterface:
    def __init__(self, master):
        ...
        self.f1_title = Label(self.f1, text="VSpipe - Virtual Screening Toolkit", font="Times 15 bold", fg="#660099")
        ...
        self.f2_outDir = Label(self.f2, text="Project Directory: ", font="Times 10 bold")
        self.f2_recep = Label(self.f2, text="Receptor File: ", font="Times 10 bold")
        self.f2_compLib = Label(self.f2, text="Library Directory: ", font="Times 10 bold")
        self.f2_outDir.grid(row=1, sticky=W,  padx=4, pady=5, ipadx=5, ipady=3)
        self.f2_recep.grid(row=2, sticky=W, padx=4, pady=5, ipadx=16, ipady=2)
        self.f2_compLib.grid(row=3, sticky=W, padx=4, pady=5, ipadx=3, ipady=3)
        ...
        self.outDir_entry = Entry(self.f2, textvariable=outDir_value, width=32)
        self.recep_entry = Entry(self.f2, textvariable=self.recep_value, width=32)
        self.compLib_entry = Entry(self.f2, textvariable=compLib_value, width=32)
        ...
        self.outDir_button = Button(self.f2, text="Define", bg="#90EE90", command=self.output_directory)
        self.recep_button = Button(self.f2, text="Load", bg="#90EE90", command=self.receptor_file)
        self.compLib_button = Button(self.f2, text="Load", bg="#90EE90", command=self.compounds_library)
        self.outDir_button.grid(row=1, column=3, padx=5, ipadx=3, pady=5)
        self.recep_button.grid(row=2, column=3, padx=5, pady=5, ipadx=6)
        self.compLib_button.grid(row=3, column=3, padx=5, pady=5, ipadx=6)

        ### -----   Buttons to open modules
        filter_results = Button(self.f2, text="Filter", bg="#FFCC33", command=self.FilterModule)
        recep_prep = Button(self.f2, text="Prepare", bg="#FFCC33", command=self.ReceptorPrepModule)
        compLib_prep = Button(self.f2, text="Prepare", bg="#FFCC33", command=self.LigandPrepModule)
        filter_results.grid(row=1, column=4, padx=5, pady=5, ipadx=8)
        recep_prep.grid(row=2, column=4, padx=5, pady=5)
        compLib_prep.grid(row=3, column=4, padx=5, pady=5)
        ...
        self.f3_xDim.grid(row=1, column=1, padx=4, pady=5)
        self.f3_yDim.grid(row=2, column=1, padx=4, pady=5)
        self.f3_zDim.grid(row=3, column=1, padx=4, pady=5)
        self.f3_xCen.grid(row=1, column=3, padx=5, pady=5)
        self.f3_yCen.grid(row=2, column=3, padx=5, pady=5)
        self.f3_zCen.grid(row=3, column=3, padx=5, pady=5)
        ...
        self.xDim_entry = Entry(self.f3, textvariable=self.xDim_value, width=18)
        self.yDim_entry = Entry(self.f3, textvariable=self.yDim_value, width=18)
        self.zDim_entry = Entry(self.f3, textvariable=self.zDim_value, width=18)
        self.xCen_entry = Entry(self.f3, textvariable=self.xCen_value, width=18)
        self.yCen_entry = Entry(self.f3, textvariable=self.yCen_value, width=18)
        self.zCen_entry = Entry(self.f3, textvariable=self.zCen_value, width=18)
        ...
        self.load_button = Button(self.f3, text="Load Parameters", bg="#90EE90", font="Times 10", command=self.load_parameters)
        self.save_button = Button(self.f3, text="Save Parameters", font="Times 10", command=self.save_parameters)
        self.load_button.grid(row=1,column=5, padx=10, pady=5)
        self.save_button.grid(row=2,column=5, padx=10, pady=5)
        ...
        self.docking_type = Label(self.f4, text="Docking Type: ", font="Times 10 bold")
        self.docking_type.grid(row=1, sticky=W, padx=3, pady=4, ipadx=7)
        self.docking_method = Label(self.f4, text="Docking Method: ", font="Times 10 bold")
        self.docking_method.grid(row=2, sticky=W, padx=3,  pady=4)
        ### -----   Define type and method of docking (Radio Buttons - blind/targeted) ----- ###
        self.dockType_value = IntVar(value=1)
        self.blind_dock = Radiobutton(self.f4, text="Blind", bg="#999999", font="Times 10", variable=self.dockType_value, value=1)
        self.targeted_dock = Radiobutton(self.f4, text="Targeted",  bg="#999999", font="Times 10", variable=self.dockType_value, value=2, command=self.dummy)
        ...
        self.vina_dock = Radiobutton(self.f4, text="Vina", bg="#999999", font="Times 10", variable=self.dockMethod_value, value=1)
        self.autodock_dock = Radiobutton(self.f4, text="Autodock",  bg="#999999", font="Times 10", variable=self.dockMethod_value, value=2, command=self.dummy)
        ...
        # Lipinki - label
        self.lipin_label = Label(self.f5, text="Lipinski's Rule: ", font="Times 10 bold")
        self.lipin_label.grid(row=1, sticky=W, padx=3, pady=1, ipadx=3)
        # Lipinski's radio buttons
        self.lipin_value = IntVar(value=1)
        self.lipin_no = Radiobutton(self.f5, text="No",  bg="#999999", font="Times 10", variable=self.lipin_value, value=1, command=self.apply_lipinRules)
        self.lipin_default = Radiobutton(self.f5, text="Default",  bg="#999999", font="Times 10", variable=self.lipin_value, value=2, command=self.apply_lipinRules)
        self.lipin_custom = Radiobutton(self.f5, text="Custom",  bg="#999999", font="Times 10", variable=self.lipin_value, value=3, command=self.apply_lipinRules)
        ...
        self.lipin_prop = Button(self.f6, text="?", fg="black", font="Times 10 bold", borderwidth=1, command=self.lipin_desc)
        ...
        self.run_button = Button(self.f7, text="RUN", fg="green", font="Times 11 bold", borderwidth=2, command=self.run)
        self.exit_button = Button(self.f7, text="EXIT", fg="red", font="Times 11", command=self.exit)
        self.clear_button = Button(self.f7, text="CLEAR", fg="blue", font="Times 11", command=self.clear_form)
    ...
    def ReceptorPrepModule(self):
        ...
        if (os.path.exists(outDir_value.get())):
            ...
            self.recepPrepWindow.geometry("390x480")
    ...
    def LigandPrepModule(self):
        ...
        if (os.path.exists(outDir_value.get())):
            ...
            self.ligPrepWindow.geometry("465x373")
    ...
    def FilterModule(self):
        ...
        if (answer):
            ...
            self.filterWindow.geometry("450x515")
            self.filterWindow.resizable(False, False)
            self.filterWindow.title("VSpipe - VS Results Filtration")
    ...
    def FilterModule(self):
        ...
        if (answer):
            ...
            if (conf):
                ...
                self.spatialWindow.geometry("450x575") # width x length
    ...
    def run(self):
        ...
        if (os.path.exists(self.recep_value.get()) and os.path.exists(compLib_value.get()) and os.path.exists(outDir_value.get())\
            ...
            self.recep_file = (self.recep_value.get()).replace("/","\\")
            ...
            self.results_dir = os.path.join(outDir_value.get(), "results")
            os.mkdir(self.results_dir)
            self.docking_results_dir = self.results_dir.replace("/","\\")
            self.lip_rules = os.path.join(self.docking_results_dir, "pdbqt_lip_rules")
            os.mkdir(self.lip_rules)
            ##...........................................................##           
            #       RUN - (Creating Datasheet - Lipinski's rule of 5 )    #
            ##...........................................................##
            
            # Copying ligands/pdbqt to results/pdbqt_lip_rules/
            os.system(f'copy {self.comp_dir_path}\* {self.lip_rules}')
            ...
            if (self.lipin_value.get() == 1):
                os.system(f'C:\Python27\python.exe C:\\vspipe-tools\datasheet.py {compLib_value.get()}\sdf\del_atoms_properties_ligands.sdf no {self.docking_results_dir} 3')
            
            # 2. choice = default, i.e., MW<500, logP<5, HBD<5, HBA<10, TPSA<150, ROT_BONDS<8
            elif (self.lipin_value.get() == 2):
                os.system(f'C:\Python27\python.exe C:\\vspipe-tools\datasheet.py {compLib_value.get()}\sdf\del_atoms_properties_ligands.sdf default {self.docking_results_dir} 3')

            # 2. choice = custom properties, e.g. "550,5.3,4,11,140,7 for MW<550, logP<5.3, HBD<4, HBA<11, TPSA<140, ROT_BONDS<7.
            else:
                os.system(f'C:\Python27\python.exe C:\\vspipe-tools\datasheet.py {compLib_value.get()}\sdf\del_atoms_properties_ligands.sdf {self.mw_value.get()},{self.logp_value.get()},{self.hbd_value.get()},{self.hba_value.get()},{self.tpsa_value.get()},{self.rtb_value.get()} {self.docking_results_dir} 3')
            ...
            if (self.dockMethod_value.get() == 1):
                ...
                # self.lip_rules = os.path.join(self.docking_results_dir, "vina_pdbqt")
                os.mkdir(os.path.join(self.docking_results_dir, "vina_pdbqt"))
                os.mkdir(os.path.join(self.docking_results_dir, "vina_log"))
                os.mkdir(os.path.join(self.docking_results_dir, "lowest_energy_pdb"))  
            ...
            if (self.dockType_value.get() == 1 and self.dockMethod_value.get() == 1):
                for lig in self.compounds_lip_rules:
                    ...
                    os.system(f'C:\\vspipe-tools\\vina.exe --receptor {self.recep_file} --ligand {self.lip_rules}\{lig} --center_x {self.xCen_value.get()} --center_y {self.yCen_value.get()} --center_z {self.zCen_value.get()} --size_x {self.xDim_value.get()}  --size_y {self.yDim_value.get()}  --size_z {self.zDim_value.get()} --out {self.docking_results_dir}/vina_pdbqt/{lig} --log {self.docking_results_dir}/vina_log/{lig}')
            ...
            # Running vina for targeted docking
                ...
                for lig in self.compounds_lip_rules:
                    ...
                    os.system(f'vina --receptor {self.recep_file} --ligand {self.lip_rules}\{lig} --center_x {self.xCen_value.get()} --center_y {self.yCen_value.get()} --center_z {self.zCen_value.get()} --size_x {self.xDim_value.get()*0.375}  --size_y {self.yDim_value.get()*0.375}  --size_z {self.zDim_value.get()*0.375} --out {self.docking_results_dir}/vina_pdbqt/{lig} --log {self.docking_results_dir}/vina_log/{lig}')
            ...
            if (self.dockMethod_value.get() == 1):
                ...
                for lig in self.path_vina_log:
                    ...
                    os.system(f'python C:\\vspipe-tools\\values_extraction_vina.py {self.docking_results_dir}/ {ligand_name}')
            
                # for renaming compound_name.pdbqt to compound_name.txt 
                for lig in self.path_vina_log:
                    ligand_name = os.path.splitext(lig)[0] + '.txt'
                    os.system(f'move {self.dr_path + lig} {self.dr_path + ligand_name}')
            ...
            if (self.dockMethod_value.get() == 2):
                ...
                self.parameter_file = self.param_file.get().replace("/","\\")
                os.system(f'copy {self.parameter_file} {cwd}\\')
                os.system(f'copy C:\\vspipe-tools\\scratch_sample.dpf {cwd}\\')
                # rename the *gpf file as sample.gpf
                os.system(f'move *gpf sample.gpf')
                ...
                os.system(f'python C:\\vspipe-tools\\generating_correct_dpf.py {cwd}\sample.gpf {cwd}\scratch_sample.dpf {cwd}\ {self.recep_file}')
                ...
                os.mkdir(os.path.join(self.docking_results_dir, "dlg"))
                os.mkdir(os.path.join(self.docking_results_dir, "gpf"))
                os.mkdir(os.path.join(self.docking_results_dir, "dpf"))
                os.mkdir(os.path.join(self.docking_results_dir, "glg"))
                os.mkdir(os.path.join(self.docking_results_dir, "summary"))
                os.mkdir(os.path.join(self.docking_results_dir, "lowest_energy_pdb"))

                os.mkdir("etc")
                # os.system(f"mkdir etc")
                os.system(f"python C:\\vspipe-tools\\creating_etc_dir.py {compLib_value.get()}\pdb\ etc\\")

                os.system(f'copy {self.recep_file} .')
                # os.system(f'cp Essential_Files/sample.gpf .')
                os.system(f'C:\\vspipe-tools\\autogrid4.exe -p sample.gpf -l sample.glg')

                for lig in self.compounds_lip_rules:
                    if (lig.endswith("pdbqt")):
                        ligand_name = os.path.splitext(lig)[0]
                        print(f"\n\nAutoDock 4.2 is analysing {ligand_name} ... ...\n")
                        os.system(f'py C:\\vspipe-tools\\prepare_gpf4.py -l {self.lip_rules}\{lig} -r {self.recep_file} -i {cwd}\sample.gpf -o {self.lip_rules}\{ligand_name}.gpf') 
                        os.system(f'py C:\\vspipe-tools\\prepare_dpf4.py -l {self.lip_rules}\{lig} -r {self.recep_file} -i {cwd}\sample.dpf -o {self.lip_rules}\{ligand_name}.dpf') 
                        os.system(f'python C:\\vspipe-tools\\dpf_rewrite.py {self.lip_rules}\{ligand_name} {cwd}\ {self.lip_rules}\\')
                    
                        # os.system(f"mkdir {self.docking_results_dir}/dlg/{ligand_name}")
                        os.mkdir(os.path.join(self.docking_results_dir, "dlg", ligand_name))
                        os.system(f'C:\\vspipe-tools\\autodock4.exe -p {self.lip_rules}\{ligand_name}.dpf -l {self.docking_results_dir}\dlg\{ligand_name}\{ligand_name}.dlg')
                        print (f"Analysis finished for {ligand_name}")

                # Moving dlg, gpf and glg files to their respective folders 
                os.system(f'move {self.lip_rules}\*.gpf {self.docking_results_dir}\gpf')
                os.system(f'move {self.lip_rules}\*.dpf {self.docking_results_dir}\dpf')
                os.system(f'move *.glg {self.docking_results_dir}\glg')
        ...
        #         #   ----------------------------------
                ...
                for file in self.docking_dlg_path: 
                    ...
                    os.system(f'py C:\\vspipe-tools\\summarize_results4.py -d {file_path} -b -o {self.docking_results_dir}\summary\summary_{file}.txt')
                    os.system(f'python C:\\vspipe-tools\\values_extraction_AD4.py {self.docking_results_dir}\ {file}')
                    print("---------------")
...
class RecepPrepModule:
    def __init__(self, master):
        ...
        self.f1_title = Label(self.f1, text="Receptor Preparation", font="Times 13 bold", fg="#660099")
        self.f1_title.pack(padx=80, pady=4)
        ...
        self.f2_pdbID = Label(self.f2, text="PDB ID: ", font="Times 10 bold")
        self.f2_OR = Label(self.f2, text="OR", font="Times 12 bold")
        self.f2_recep = Label(self.f2, text="Receptor: ", font="Times 10 bold")
        self.f2_pdbID.grid(row=1, sticky=W, padx=7, ipadx=5, pady=5)
        self.f2_OR.grid(row=2, column=2, padx=20, pady=5)
        self.f2_recep.grid(row=3, sticky=W, padx=7, ipadx=2, pady=5)

        ### -----   Entry Widgets for Inputing file ----- ###
        self.recep_value = StringVar()
        self.pdbID_value = StringVar()
        self.pdbID_entry = Entry(self.f2, textvariable=self.pdbID_value, width=30)
        # myTip = Hovertip(self.pdbID_entry,'e.g., 3e7a')
        self.recep_entry = Entry(self.f2, textvariable=self.recep_value, width=30)
        # myTip = Hovertip(self.recep_entry,'Enter the file path')
        self.pdbID_entry.grid(row=1, column=2, padx=7, pady=5)
        self.recep_entry.grid(row=3, column=2, padx=7, pady=5)
        ...
        self.pdbID_button.grid(row=1, column=3, padx=6, ipadx=5, pady=5)
        self.recep_button.grid(row=3, column=3, padx=6, pady=5)
        ...
        self.radio_default = Radiobutton(self.f3, text="Keep the file by default", font="Times 10", variable=self.radio_value, value=1, command=self.keep_metalIon)
        # myTip = Hovertip(self.radio_default,'Do nothing')
        self.radio_firstChain = Radiobutton(self.f3, text="Extract first chain and No metal ion", font="Times 10", variable=self.radio_value, value=2, command=self.keep_metalIon)
        # myTip = Hovertip(self.radio_firstChain,'Remove water, ligand(s) and metal ion(s)')
        self.radio_metalIon = Radiobutton(self.f3, text="Extract first chain and keep the metal ion", font="Times 10", variable=self.radio_value, value=3, command=self.keep_metalIon)
        # myTip = Hovertip(self.radio_metalIon,'Remove water, ligand(s) but keep specified metal ion')
        self.radio_default.grid(row=2, sticky=W, padx=7, pady=7)
        self.radio_firstChain.grid(row=3, sticky=W, padx=7, pady=7)
        self.radio_metalIon.grid(row=4, sticky=W, padx=7, pady=7)
        ...
        self.f4_metal_ion = Label(self.f4, text="Specify Metal Ion: ", font="Times 10")
        self.f4_metal_charge = Label(self.f4, text="Specify Metal Charge: ", font="Times 10")
        self.f4_metal_ion.grid(row=1, sticky=W, padx=7, ipadx=10, pady=5)
        self.f4_metal_charge.grid(row=2, sticky=W, padx=7, pady=5)
        ...
        ### -----   Buttons for running the script, clearing and exiting the program ----- ###
        self.run_button = Button(self.f5, text="RUN", fg="green", font="Times 10 bold", borderwidth=2, command=self.run)
        # myTip = Hovertip(self.run_button,'To generate PDBQT')
        self.exit_button = Button(self.f5, text="EXIT", fg="red", font="Times 10", command=self.terminate)
        self.run_button.grid(row=3, column=6, padx=8)
        self.exit_button.grid(row=3, column=8, padx=8)
    ...
    def retreive_pdb(self):   
        ...
        os.system(f'move {self.downloaded_file} {self.pdb_file}')
    ...
    def process_metallo_receptor(self, generated_file):
        ...
        self.pdbcode = self.generated_file[0:4]
        # Renaming the pdbid_clean.pdbqt with pdbid.pdbqt
        os.system(f'move {self.generated_file} {self.pdbcode}.pdbqt')
        # Correcting generated pdbqt file of metalloprotein
        os.system(f'python C:\\vspipe-tools\\receptor_pdbqt_correction.py {self.pdbcode}')
        # Adding charge in corrected pdbqt file of metalloprotein
        os.system(f'python C:\\vspipe-tools\\adding_metal_charge.py {self.pdbcode} {self.metalIon_value.get()} {self.metalCharge_value.get()}')
    ...
    def process_receptor(self, receptor):
        ...
        os.system(f'py C:\\vspipe-tools\\prepare_receptor4.py -r {self.receptor} -A hydrogens -U nphs waters')
    ...
    def run(self):
        ...
        os.mkdir(os.path.join(outDir_value.get().replace("/","\\"), "receptor"))
        self.receptor = self.recep_value.get().replace("/","\\")
        print("#####", self.receptor)
        ...
        if (os.path.isfile(self.pdbID_value.get())):
            ...
            print("receptor file: ", self.receptor)
            # To extract the filename from the browsed file path
            self.selected_receptor = os.path.basename(f'{self.receptor}')
            print("@@@@", self.selected_receptor)
            # Copy the receptor file in the present working directory
            cwd = os.getcwd()
            print("Present Working Directory", cwd)
            os.system(f'copy {self.receptor} {cwd}')
        else:
        ...
        if (self.radio_value.get() == 1):  
            ...
            os.system(f'py C:\\vspipe-tools\\prepare_receptor4.py -r {self.selected_receptor} -A hydrogens -U nphs waters')
        ...
        if (self.radio_value.get() == 2):  
            ...
            os.system(f'python C:\\vspipe-tools\\clean_protein.py {self.selected_receptor}')
            # os.system(f"awk '!/HOH/' {selected_receptor} > clean_{selected_receptor}")
            # Converting receptor to pdbqt
            os.system(f'del {self.selected_receptor}')
            os.system(f'move *pdb clean_{self.selected_receptor}')
            self.directory = os.listdir()
        ...
        if (self.radio_value.get() == 3):  
            ...
            os.system(f'python C:\\vspipe-tools\\adding_metal_ion.py {self.selected_receptor} {self.metalIon_value.get()}')
            # removes temporary files
            # os.system(f'rm noWater_{self.selected_receptor} ChainA_{self.selected_receptor}')
            # Converting receptor to pdbqt
            self.directory = os.listdir()
            for self.receptor in self.directory:
                if (self.receptor.endswith('clean.pdb')):
                    # os.system(f'gedit {receptor}')
                    self.process_receptor(self.receptor)
        
        # move all the receptor related files to receptor directory
        os.system(f'move *pdb* {outDir_value.get()}/receptor')
...
class LigandPrepModule:
    def __init__(self, master):
        ...
        self.f1 = Frame(self.master, borderwidth=5, relief=SUNKEN)
        self.f1.grid(row=1, column=3, columnspan=2, padx=55, pady=5)
        # Frame Title
        self.f1_title = Label(self.f1, text="Compounds Library Preparation", font="Times 12 bold", fg="#660099")
        self.f1_title.grid(row=1, column=3, padx=55, ipadx=30)
        ...
        self.tab1_label = Label(self.tab1, text="Please select following minimized compounds library:", font="Times 10 bold")
        self.tab1_label.grid(row=1, column=1, sticky=W, padx=12, pady=15)
        ...
        self.tab2_label = Label(self.tab2, text="Conversion and Minimization of file in sdf/mol2 format", font="Times 10 bold")
        self.tab2_label.grid(row=1, sticky=W, padx=5, pady=15)
        ...
        self.radio_flag1 = Radiobutton(self.tab2, text="File Format: PDB, MOL, MOL2", variable=self.radio_value, value=1)
        self.radio_flag2 = Radiobutton(self.tab2, text="File Format: CAN, SMI", variable=self.radio_value, value=2)
        self.radio_flag3 = Radiobutton(self.tab2, text="File Format: SDF", variable=self.radio_value, value=3)
    ...
    def Libaray_Selection(self):
        ...
        self.lib_path = 'C:\\minimised_libs/' + self.value
    ...
    def batch_file_minimzation(self):
        ...
        if (self.radio_value.get() == 1 or self.radio_value.get() == 2):
            ...
            os.system(f'C:\OpenBabel\obabel.exe {self.filepath} -O ligands\sdf\ligands.sdf')
            ...
            os.system(f'C:\OpenBabel\obabel.exe ligands\sdf\ligands.sdf -O ligands\sdf\properties_ligands.sdf -b --unique --add cansmi HBA2 HBD logP TPSA MW')
        
        # File Format: SDF 
        if (self.radio_value.get() == 3):
                      
            # Producing canonical smiles and missing properties of the ligands and saving it as ligands/sdf/properties_ligands.sdf
            os.system(f'C:\OpenBabel\obabel.exe {self.filepath} -O ligands\sdf\properties_ligands.sdf -b --unique --add cansmi HBA2 HBD logP TPSA MW')
        ...
        os.system(f'C:\Python27\python.exe C:\\vspipe-tools\\atom_deletion.py ligands\sdf\properties_ligands.sdf ligands\sdf\\')

        # Minimizing ligands/sdf/del_atoms_properties_ligands.sdf using openBabel
        os.system(f'C:\OpenBabel\obabel.exe ligands\sdf\del_atoms_properties_ligands.sdf -O ligands\pdb\ligand.pdb --gen3d -m --conformer --nconf 50 --score energy')
        ...
        os.system(f'C:\Python27\python.exe C:\\vspipe-tools\\pdbs_rename.py {self.library_path_sdf}\del_atoms_properties_ligands.sdf {self.library_path} 3') 
        ...
        for self.lig in self.lig_path:
            # Change the path to the directory where compounds (*pdb) are present (Windows version specific)
            os.chdir(f'{self.library_path}')
            print(f'Adding hydrogens in the {self.lig} file')
            print('Merging charges and removing non-polar hydrogens')
            os.system(f'py C:\\vspipe-tools\\prepare_ligand4.py -l {self.lig} -A hydrogens -U nphs')
            # moving the generated pdbqts to /ligands/pdbqt/ directory
            os.system('move *pdbqt ..\pdbqt\\')
        
        # Message dialogue for succesful minimization of the compounds library 
        self.library_path= os.getcwd() + "qt\\"
    ...
    def pdbDir_to_pdbqt(self):
        ...
        for self.lig in self.lig_path:
            ...
            print('Merging charges and removing non-polar hydrogens')
            os.system(f'py C:\\vspipe-tools\\prepare_ligand4.py -l {self.library_path + self.lig} -A hydrogens -U nphs')
        # moving the generated pdbqts to /ligands/pdbqt/ directory
        os.system('move *pdbqt .\ligands\pdbqt\\')
    ...
    def run(self):
        ...
        if (self.tab == 2 or self.tab == 3):
            os.chdir(f'{outDir_value.get()}')
            # Create directories for saving respective ligands files        
            os.mkdir(os.path.join(outDir_value.get().replace("/","\\"), "ligands"))
            os.mkdir(os.path.join(outDir_value.get().replace("/","\\"), "ligands\sdf"))
            os.mkdir(os.path.join(outDir_value.get().replace("/","\\"), "ligands\pdb"))
            os.mkdir(os.path.join(outDir_value.get().replace("/","\\"), "ligands\pdbqt"))
...
class FiltrationModule:
    def __init__(self, master):
        ...
        self.f1_title = Label(self.f1, text="VS Results Filtering", font="Times 15 bold", fg="#660099")
        self.f1_title.grid(row=1, column=5, sticky=W, padx=80)
        ...
        ### -----   Labels for Inputing file ----- ###
        self.vsResults_label = Label(self.f2, text="VS Results Directory: ", font="Times 10 bold")
        self.vsResults_label.grid(row=1, sticky=W, padx=5, pady=5, ipadx=1)
        
        self.vsResults_value = StringVar()
        self.vsResults_entry = Entry(self.f2, textvariable=self.vsResults_value, width=28)
        self.vsResults_entry.grid(row=1, column=2, padx=10, pady=5)
        
        ### -----   Browse buttons for Inputing file (Ready)----- ###
        self.vsResults_button = Button(self.f2, text="Browse", command=self.browse_vsResults_dir)
        self.vsResults_button.grid(row=1, column=3, padx=5, pady=5)
        ...
        ### -----   Labels for Inputing file ----- ###
        self.parmOrder_label = Label(self.f3, text="Sort the results by:  ", font="Times 10 bold")
        self.parmOrder_label.grid(row=1, sticky=W, padx=5, pady=5, ipadx=9)

        # Combobox dropdown menu
        self.param_value = StringVar(value=' 8. δG')
        self.param_combo = ttk.Combobox(self.f3, width=25, textvariable = self.param_value)
        ...
        self.param_combo.grid(column=2, row=1, padx=11, sticky=W, pady=10)
        ...
        # Yes/No Filter
        self.filter_label = Label(self.f3, text="Apply Filter:  ", font="Times 10 bold")
        self.filter_label.grid(row=2, sticky=W, padx=5, pady=20, ipadx=28)

        # Yes/No filter radio buttons
        self.filter_value = IntVar(value=1)
        self.filter_no = Radiobutton(self.f3, text="No",  bg="#999999", font="Times 10", variable=self.filter_value, value=1, command=self.filtraton_option)
        self.filter_yes = Radiobutton(self.f3, text="Yes",  bg="#999999", font="Times 10", variable=self.filter_value, value=2, command=self.filtraton_option)
        ...
        self.filter_1 = Radiobutton(self.f4, text="Filter 1",  bg="#999999", font="Times 10", variable=self.choice_value, value=1, command=self.filter_choice)
        self.filter_2 = Radiobutton(self.f4, text="Filter 2",  bg="#999999", font="Times 10", variable=self.choice_value, value=2, command=self.filter_choice)
        self.filter_3 = Radiobutton(self.f4, text="Filter 3",  bg="#999999", font="Times 10", variable=self.choice_value, value=3, command=self.filter_choice)
        ...
        self.filter_1_label = Label(self.f4, text="Greater than:  ", font="Times 10", bg="#999999")
        ...
        self.filter_2_label = Label(self.f4, text="Less than:  ", font="Times 10", bg="#999999")
        ...
        self.filter_3_min = Label(self.f4, text="Range - Min:  ", font="Times 10", bg="#999999")
        self.filter_3_max = Label(self.f4, text="Range - Max:  ", font="Times 10", bg="#999999")
        ...
        ### -----   Buttons for running the script, clearing and exiting the program ----- ###
        self.run_button = Button(self.f5, text="RUN", fg="green", font="Times 11 bold", borderwidth=2, command=self.run)
        self.exit_button = Button(self.f5, text="EXIT", fg="red", font="Times 11", command=self.terminate)
        self.clear_button = Button(self.f5, text="CLEAR", fg="blue", font="Times 11", command=self.dummy)
        self.run_button.grid(row=3, column=6, padx=8)
        self.exit_button.grid(row=3, column=8, padx=8)
        self.clear_button.grid(row=3, column=4, padx=8)
    ...
    def run(self):
        ...
        # C:\\vspipe-tools\\prepare_ligand4.py
        self.results_dir = self.vsResults_value.get().replace("/","\\") + "\\"
        os.mkdir(os.path.join(self.results_dir, "filtered_results"))
        ...
        if (self.filter_value.get() == 1):  
            ...
            os.system(f'Rscript.exe C:\\vspipe-tools\\filtering.R {self.results_dir} {self.param_combo.current()+1} null 0 {self.results_dir}\\filtered_results')
            
            # Move the generedted files to filtered_results/ 
            os.system(f'move {self.results_dir}*pdf {self.results_dir}\\filtered_results/')    
            os.system(f'move {self.results_dir}order* {self.results_dir}\\filtered_results/') 
        ...
        if (self.filter_value.get() == 2): 
            ...
            os.mkdir(os.path.join(self.results_dir,"filtered_results", "filtered_pdbs"))
       

            # Filter 1: Arrange output.tsv according to the selected parameter as (values > selected choice)
            if(self.choice_value.get() == 1):
                os.system(f'Rscript.exe C:\\vspipe-tools\\filtering.R {self.results_dir} {self.param_combo.current()+1} ">{self.filter_1_value.get()}" 1 {self.results_dir}\\filtered_results/')
                
            # Filter 2: Arrange output.tsv according to the selected parameter as (values < selected choice)
            if(self.choice_value.get() == 2):
                os.system(f'Rscript.exe C:\\vspipe-tools\\filtering.R {self.results_dir} {self.param_combo.current()+1} "<{self.filter_2_value.get()}" 1 {self.results_dir}\\filtered_results/')
                
            # Filter 3: Arrange output.tsv according to the selected parameter as (range (min,max))
            if(self.choice_value.get() == 3):
                os.system(f'Rscript.exe C:\\vspipe-tools\\filtering.R {self.results_dir} {self.param_combo.current()+1} "{self.filter_3_min_value.get()},{self.filter_3_max_value.get()}" 1 {self.results_dir}\\filtered_results/')

            # Move the filtered pdbs to ../filtered_results/filtered_pdbs/ 
            os.system(f'move {self.results_dir}\\filtered_results\*pdb {self.results_dir}\\filtered_results\\filtered_pdbs\\')  
...
if __name__ == '__main__':
    ...
    root = Tk()
    root.geometry("515x625") # width x length
